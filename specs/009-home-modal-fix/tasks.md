...
- [x] T001 [CTX7] Read/refresh Vue 3 Composition API docs for `computed`, props, emits, and custom `v-model` patterns using Context7.
- [x] T002 [CTX7] Read/refresh PrimeVue Dialog docs for `v-model:visible` using Context7.
- [x] T003 [CTX7] Read/refresh PrimeVue DataTable row click/styling docs using Context7.
- [x] T004 [CTX7] Read/refresh Vue Test Utils / Vitest component testing docs using Context7.
- [x] T005 [CTX7] Read/refresh Playwright locator, screenshot, trace, download, popup/new-tab, and clipboard docs using Context7.
- [x] T006 Inspect `frontend/src/components/home/ResumeDetailsDialog.vue`; confirm whether it uses `ref(props.visible)` or another broken one-time prop copy.
- [x] T007 Inspect `frontend/src/views/UserHomePage.vue`; confirm table and summary card both route into `openResumeModal`.
- [x] T008 Inspect `frontend/src/components/home/SavedResumesTable.vue`; confirm row-click behavior and current row styling.
- [x] T009 Inspect `frontend/src/components/home/SummaryCards.vue`; confirm latest card click behavior and whether clickable styling is conditional.
- [x] T010 Inspect `frontend/src/services/userHomeService.ts`; document current `SavedResumeData` fields.
- [x] T011 Inspect backend list/summary APIs: `ResumeController`, `UserHomeController`, `ResumeService`, `UserHomeService`, DAO methods.
- [x] T012 Inspect current public route: `PublicResumeController` and related DAO/service lookup methods.
- [x] T013 Inspect current delete behavior; determine whether both `is_deleted` and `deleted_at` are updated.
- [x] T014 Inspect existing error pages. Confirm that `404.html` and `500.html` exist. Check whether `templates/error/410.html` exists.
- [x] T015 Inspect current config mechanism and decide how backend can read `APP_PUBLIC_BASE_URL`. Do not assume `.env` is automatically read in plain Spring MVC.
- [x] T016 Inspect `.env.example` and `.gitignore`; document whether `APP_PUBLIC_BASE_URL` and `tempfiles/` are present.
- [x] T017 Inspect current export endpoints: verify `GenerateResumeController` serves `GET /api/generate/resumes/{id}/pdf`, `/pdf?disposition=inline`, `/html`. Confirm these are the canonical endpoints before using them in `HomeSavedResumeDto`. If the actual routes differ, STOP and report before changing the DTO contract.
- [x] T018 Run grep baseline:
...
- [x] T019 [STOP] [EVIDENCE] Report baseline findings to the user:
...
- [x] T020 [TDD] Add tests for `PublicUrlService`:
...
- [x] T021 Add `.env.example` entry:

```env
APP_PUBLIC_BASE_URL=http://localhost:8080
```

- [x] T022 Ensure `.env` is ignored by git if present.
- [x] T023 Implement `PublicUrlService` (NEW) in `backend/src/main/java/com/resumainer/service/PublicUrlService.java`:
...
- [x] T024 [SEC] Do not add a new dotenv library unless the project already uses one.

### 1.2 Home safe DTO

- [x] T025 [TDD] Add backend tests for canonical `HomeSavedResumeDto` shape and mapping.
- [x] T026 Create `HomeSavedResumeDto` (NEW) in `backend/src/main/java/com/resumainer/dto/home/HomeSavedResumeDto.java` with fields:
...
- [x] T027 Implement shared mapper method used by both paginated list and `summary.lastResume`. Map canonical export endpoints from `GenerateResumeController` (not `ResumeDownloadController` legacy routes).
- [x] T028 Update `ResumeController.listResumes()` response to use `HomeSavedResumeDto`.
- [x] T029 Update `UserHomeService.getHomeSummary()` so `summary.lastResume` uses the same `HomeSavedResumeDto` mapper. Ensure it lives under `summary.lastResume`, not at root level.
- [x] T030 [SEC] Ensure response never includes: `pdf_file_path`, `html_file_path`, local filesystem path, or raw storage directory.

- [x] T030A [STOP] [EVIDENCE] Backend DTO/config checkpoint. Stop and report before continuing to delete/public-route work:
...
- [x] T031 [TDD] Add/update delete test: successful delete sets both `is_deleted=true` and `deleted_at`.
- [x] T032 Update `ResumeDao` soft-delete to set both `is_deleted = true` and `deleted_at = CURRENT_TIMESTAMP` in one consistent update.
- [x] T033 Ensure Home paginated list excludes deleted resumes (`is_deleted = false` in WHERE clause).
- [x] T034 Ensure `summary.lastResume` excludes deleted resumes.
- [x] T035 [SEC] Implement delete endpoint response policy:
...
- [x] T035A [STOP] [EVIDENCE] Delete/list consistency checkpoint. Stop and report before continuing to public-route/410 work:
...
- [x] T036 [TDD] Add/update public route tests:
...
- [x] T037 Create `PublicResumeLookupResult` (NEW) with status values: `ACTIVE`, `DELETED`, `NOT_FOUND`, `MISSING_FILE`, `UNSAFE_PATH`.
- [x] T038 Update `PublicResumeController`:
...
- [x] T039 [SEC] Apply the same existing uniform artificial delay mechanism to both `404` and `410` error responses. Prefer reusing an existing helper/service/filter. If no existing delay mechanism is found, STOP and ask before inventing a new timing value, `Thread.sleep`, filter, or helper.

### 1.5 Thymeleaf 410 page

- [x] T040 Inspect existing `templates/error/404.html` and `templates/error/500.html` for style reference.
- [x] T041 Create `backend/src/main/resources/templates/error/410.html` if missing, following existing error page style. Content must be static branded text and i18n strings only. No dynamic resume data, username, public code, deletion date, IDs, paths, filenames, company/vacancy data.
- [x] T042 Add i18n messages to `messages.properties` and `messages_ru.properties`:
...
- [x] T043 Ensure deleted public route renders/forwards to Thymeleaf `410.html` page with HTTP status `410`.

- [x] T043A [STOP] [EVIDENCE] Public route + 410 checkpoint. Stop and report before backend aggregation tests:
...
- [x] T044 Run targeted backend tests:
...
- [x] T045 Run broader backend tests if targeted pass:
...
- [x] T046 [EVIDENCE] Save sanitized API sample responses under `tempfiles/Playwright_evidence/backend_api_samples/`. Do not include secrets or real PII.
- [x] T047 [STOP] [EVIDENCE] Report backend checkpoint to user:
  - changed files list;
...
- [x] T048 [TDD] Update/add frontend tests expecting canonical saved resume fields.
- [x] T049 Update `frontend/src/services/userHomeService.ts`:
...
- [x] T050 Update all consuming TypeScript code to compile with new fields. Ensure `publicUrlLink` is not treated as old-field.

### 2.2 ResumeDetailsDialog — v-model fix

- [x] T051 [TDD] Add test: parent sets `visible=true` -> Dialog opens.
- [x] T052 [TDD] Add test: close emits `update:visible=false`.
- [x] T053 [CTX7] Confirm Vue 3 custom `v-model` computed bridge pattern through Context7 if uncertain.
- [x] T054 Fix modal visibility binding. Replace `ref(props.visible)` with computed get/set bridge:
...
- [x] T054A [STOP] [EVIDENCE] Frontend modal opening checkpoint. Stop and report before implementing modal actions/delete:
...
- [x] T055 Update modal content to display:
...
- [x] T056 [SEC] Modal MUST NOT expose raw filesystem paths.
- [x] T057 Handle clipboard API failure gracefully — show user-readable error feedback via toast.
- [x] T058 Ensure long modal metadata text (title, vacancy, company) wraps within grid cells without truncation (word-wrap/break).

### 2.4 ResumeDetailsDialog — delete flow

- [x] T059 [TDD] Add test: delete button exists and opens confirmation.
- [x] T060 [TDD] Add test: cancel delete keeps resume unchanged.
- [x] T061 [TDD] Add test: confirm delete triggers delete API call.
- [x] T062 [TDD] Add test: after successful delete, modal closes and reload functions are called.
- [x] T063 [TDD] Add test: delete API failure keeps modal open and shows generic error toast.
- [x] T064 Add delete action in modal:
...
- [x] T064A [STOP] [EVIDENCE] Frontend modal actions/delete checkpoint. Stop and report before row styling/summary/i18n cleanup:
...
- [x] T065 [TDD] Add test: row click emits selected resume for modal.
- [x] T066 [TDD] Add test: no Details button/column is rendered.
- [x] T067 [CTX7] If DataTable row styling is unclear, check PrimeVue DataTable docs through Context7.
- [x] T068 Add pointer cursor and hover highlight for clickable rows via CSS class.
- [x] T069 Ensure no extra Details column/button is added.

### 2.6 SummaryCards latest resume behavior

- [x] T070 [TDD] Add test: latest card emits `openLastResume` only when `lastResume` exists.
- [x] T071 [TDD] Add test: latest card is not visually clickable when `lastResume` is null.
- [x] T072 Update `SummaryCards.vue` so clickable styling applies only when `lastResume` exists. No `openLastResume` emit when absent.
- [x] T073 Ensure `UserHomePage.vue` correctly passes `summary.lastResume` into `openResumeModal`.

### 2.7 i18n

- [x] T074 Add RU and EN i18n keys for all new visible strings in `frontend/src/i18n/en.json` and `ru.json`:
...
- [x] T075 Ensure no hardcoded visible user-facing strings remain in modified Vue components.

### 2.8 Frontend evidence

- [x] T076 Run frontend tests:
...
- [x] T079 [STOP] [EVIDENCE] Report frontend checkpoint to user:
  - changed files list;
  - modal v-model fix;
...
- [x] T080 Create local evidence folder:

```bash
mkdir -p tempfiles/Playwright_evidence
```

- [x] T081 Confirm `tempfiles/` is ignored by git. If not, update `.gitignore`.

### 3.2 Manual prerequisite stop

- [x] T082 [STOP] Ask the user to manually generate a new disposable resume for a throwaway/local test user. Warn clearly that the Playwright delete scenario will intentionally delete this generated resume.
...
- [x] T083 [STOP] Ask the user to provide:
  - local app base URL;
  - throwaway/local test login;
  - throwaway/local test password;
  - whether the generated test resume includes cover letter.
- [x] T084 Do not write the credentials into files. Do not commit credentials or evidence.

### 3.3 Playwright scenario — modal and actions

- [x] T085 [PW] Log in as the throwaway/local test user.
- [x] T086 [PW] Open Home page.
- [x] T087 [PW] Verify saved resumes table includes the newly generated resume.
- [x] T088 [PW] Hover over table row; capture screenshot showing hover/pointer visual if possible.
- [x] T089 [PW] Click table row; verify modal opens for the selected row (not latest by mistake).
- [x] T090 [PW] Capture screenshot:

```text
tempfiles/Playwright_evidence/01_table_row_modal_open.png
```

- [x] T091 [PW] Verify modal metadata: vacancy title, company if present, saved resume creation date shown as generation date, language, adaptation level.
- [x] T092 [PW] Verify visible public URL text (full absolute URL).
- [x] T093 [PW] Click Copy public link; verify success feedback or clipboard content if available.
- [x] T094 [PW] Click Open PDF; verify new tab/popup or inline PDF route response.
- [x] T095 [PW] Capture screenshot or trace:

```text
tempfiles/Playwright_evidence/02_open_pdf.png
```

- [x] T096 [PW] Click Download PDF; verify download event/request.
- [x] T097 [PW] Click Download HTML; verify download event/request.
- [x] T098 [PW] If cover letter exists:
...
- [x] T099 [PW] If cover letter does not exist:
  - verify exact empty-state text: `Сопроводительное письмо не было выбрано в настройках генерации.`
- [x] T100 [PW] Close modal via close icon/Escape/backdrop.
- [x] T101 [PW] Click latest-resume summary card.
- [x] T102 [PW] Verify same modal opens for latest resume.
- [x] T103 [PW] Capture screenshot:
...
- [x] T104 [PW] Copy/store the public URL locally for the test session only.
- [x] T105 [PW] Open modal delete action.
- [x] T106 [PW] Verify confirmation dialog appears.
- [x] T107 [PW] Cancel delete first; verify resume remains and modal stays usable.
- [x] T108 [PW] Open delete action again and confirm delete.
- [x] T109 [PW] Verify confirm button disables with loading state.
- [x] T110 [PW] Verify modal closes after successful soft-delete.
- [x] T111 [PW] Verify table refreshes and deleted resume is gone.
- [x] T112 [PW] Verify summary latest card refreshes to next active resume or empty state.
- [x] T113 [PW] Open previously copied public URL.
- [x] T114 [PW] Verify HTTP status is `410 Gone`.
- [x] T115 [PW] Verify backend Thymeleaf 410 page is shown, not Vue SPA.
- [x] T116 [PW] Verify text: `Пользователь решил удалить данное резюме. Больше оно не доступно.`
- [x] T117 [PW] Verify 410 page contains no resume-specific data (no username, public code, ID, dates, paths, vacancy, company).
- [x] T118 [PW] Capture screenshot:
...
- [x] T119 If any Playwright step fails:
  - save screenshot;
...
- [x] T120 [STOP] [EVIDENCE] Report Playwright checkpoint to user:
...
- [x] T121 [NO-PDF-ENGINE] Run backend tests:
...
- [x] T125 Verify git status does not include:
  - `tempfiles/Playwright_evidence/`;
  - screenshots;
  - traces;
  - test credentials;
  - generated resume content.
- [x] T126 [NO-PDF-ENGINE] Verify no unrelated PDF engine/fitting/template files were modified. If any were modified, justify explicitly or revert.
- [x] T127 Update final task report with:
...
- [x] T128 [STOP] [EVIDENCE] Final handoff to user. Do not claim completion without evidence.
...
- [x] Table row click opens modal for selected resume.
- [x] No Details column/button added to table.
- [x] Rows show pointer cursor and hover highlight.
- [x] Latest-resume card opens modal for `summary.lastResume`.
- [x] Latest card is not clickable when `lastResume` absent.
- [x] Modal uses canonical fields (`publicUrlLink`, `pdfOpenUrl`, `pdfDownloadUrl`, `htmlDownloadUrl`, `pdfAvailable`, `coverLetter`), not old `pdfUrl`/`publicUrl`.
- [x] Modal shows saved resume creation date as generation date, vacancy title, company if present, language, adaptation level.
- [x] Modal shows public URL text and Copy public link action.
- [x] Modal opens PDF in new tab when available (uses `pdfOpenUrl`).
- [x] Modal downloads PDF when available (uses `pdfDownloadUrl`).
- [x] Modal downloads HTML for authenticated owner when available (uses `htmlDownloadUrl` from canonical `GenerateResumeController` endpoints).
- [x] Modal disables PDF buttons and shows message when `pdfAvailable=false`.
- [x] Modal shows cover letter and Copy cover letter button when present (copy = full text).
- [x] Modal shows approved i18n empty-state text when cover letter absent.
- [x] Modal has delete action with confirmation dialog.
- [x] Delete confirm button disables with loading state (double-click prevention).
- [x] Cancel delete keeps resume unchanged.
- [x] Successful delete closes modal, reloads table, reloads summary.
- [x] Delete API failure keeps modal open with generic error toast.
- [x] Public URL is computed from `APP_PUBLIC_BASE_URL` or forwarded-header/request-origin fallback.
- [x] `.env.example` includes `APP_PUBLIC_BASE_URL`.
- [x] `PublicUrlService` logs warning when fallback origin is used.
- [x] Soft delete sets both `is_deleted` and `deleted_at`.
- [x] Deleted public link returns HTTP `410 Gone` with backend Thymeleaf page.
- [x] `410.html` contains no dynamic resume data, username, public code, IDs, dates, paths, or company/vacancy.
- [x] Invalid/unsafe/missing public link returns HTTP `404`.
- [x] `404` and `410` responses have same uniform artificial delay (reuse existing mechanism).
- [x] Public route does not intercept `/api/**`, `/app/**`, `/static/**`, `/assets/**`, `/error/**`, landing page, or Vue SPA assets.
- [x] No raw `pdf_file_path`/`html_file_path`/storage directory exposed in API responses.
- [x] No old standalone `publicUrl` / `pdfUrl` frontend usage remains.
- [x] RU/EN i18n strings present for all new UI text.
- [x] Backend tests pass (`mvn test`).
- [x] Frontend tests pass (`npm test -- --run`).
- [x] Frontend build passes (`npm run build`).
- [x] Playwright MCP evidence exists in `tempfiles/Playwright_evidence/`.
- [x] Evidence files and credentials are not committed.
- [x] No modified PDF renderer/fitting/finalization/template files.
- [x] User confirmed all stop points.
