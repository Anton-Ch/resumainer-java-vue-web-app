...
3. Create minimal `SecurityConfig`.
4. Ensure app starts.
5. Keep endpoints temporarily permissive if needed for bootstrap only.
6. Add tests proving Spring Security filter chain is active.
...
1. Add users auth columns.
2. Add `auth_tokens`.
3. Add `oauth_accounts`.
4. Add `persistent_logins`.
5. Add existing test-user migration to:
   - mark email verified;
   - set encoded `Aa123456`;
   - preserve roles/status/permissions/deletion flags.
6. Add DAO tests/migration evidence where project style supports it.
...
7. Update tests.
8. Verify frontend login/logout/status.
...
8. Add resend verification endpoint.
9. Add cooldown/hour/day rate limits.
10. Add tests.
...
4. Create hashed 15-minute token for existing eligible user.
5. Send reset email.
6. Add validate token endpoint.
7. Add confirm reset endpoint.
8. Consume token.
9. Invalidate other active reset tokens for user.
...
2. Add frontend Continue with Google button.
3. Start flow through `/oauth2/authorization/google`.
...
10. Update EN/RU i18n.
11. Proofread landing copy.
