...
* **FR-107**: Cloudflare Turnstile MUST protect registration.
* **FR-108**: Cloudflare Turnstile MUST protect forgot-password request.
* **FR-109**: Cloudflare Turnstile MUST protect resend verification.
* **FR-110**: Captcha MUST be required for login after suspicious/failed attempts.
* **FR-111**: Captcha failure MUST show a clear user-facing message.
* **FR-112**: Dev mode MUST support controlled captcha testing without real Turnstile.
* **FR-113**: Production mode MUST NOT accept dev captcha bypass.
* **FR-114**: Production mode MUST fail safely or clearly report misconfiguration if captcha secrets are missing.
* **FR-115**: Public auth rate limits MUST be enforced server-side.

### Email Delivery

* **FR-116**: Email delivery MUST use Resend.
* **FR-117**: Production mode MUST require Resend API key.
* **FR-118**: Production mode MUST NOT silently no-op email delivery.
* **FR-119**: Development mode MAY log email links if Resend API key is absent.
* **FR-120**: Email templates MUST include plain text and basic HTML.
* **FR-121**: Email templates MUST be bilingual, with English first and Russian second.
* **FR-122**: Verification and reset links MUST use configured public base URLs.
* **FR-123**: Backend verification links MUST use backend public base URL.
* **FR-124**: Password reset links MUST use frontend public base URL.

### Admin Account Visibility

* **FR-125**: Admin User Details → Account tab MUST show email verification status.
* **FR-126**: Admin Users table MUST NOT add an email verification column in this feature.
* **FR-127**: Only admins MUST be able to view admin account verification status.
* **FR-128**: Admin DTOs MUST NOT expose password hashes, OAuth provider secrets, tokens, token hashes, API keys, or internal security details.

### i18n and Copy

* **FR-129**: All new frontend auth text MUST be localized in English and Russian.
* **FR-130**: All new auth error messages MUST be localized in English and Russian.
* **FR-131**: Auth copy MUST be natural, user-friendly, and free of raw i18n keys.
* **FR-132**: Landing page copy MUST be proofread for obvious grammar, typos, and unnatural wording.
* **FR-133**: OAuth unverified-email rejection MUST have a clear production-grade explanation.
* **FR-134**: Captcha-required messages MUST be understandable and not overly technical.

### Production and Public Surface Hardening

* **FR-135**: Captcha MUST NOT be documented as DDoS protection for GET landing.
* **FR-136**: Landing protection MUST be documented as proxy/Nginx rate limiting, static caching, and request/body limits.
* **FR-137**: Production cookies MUST be configured safely for HTTPS deployment.
* **FR-138**: Secrets MUST be provided through environment/configuration and MUST NOT be included in frontend assets.
* **FR-139**: Missing production secrets MUST fail safely or produce clear configuration errors.
* **FR-140**: Public endpoint abuse protection MUST be covered by tests or manual evidence.

### Non-Regression

* **FR-141**: The feature MUST NOT modify PDF rendering, PDF fitting, PDF validation, PDF templates, AI generation, prompt building, parser, OpenRouter client, finalization pipeline, or budget config.
* **FR-142**: The feature MUST NOT hard-delete users, resumes, profile data, or generated files.
* **FR-143**: Existing User Home, Profile, Generate Resume, Public Resume, and Admin flows MUST continue to work after auth migration.
* **FR-144**: Existing tests for unrelated domains MUST not be weakened or removed to hide regressions.
* **FR-145**: New/changed security code MUST be covered by positive, negative, boundary, and abuse-case tests.
...
> This API draft exists to make the feature testable and to prevent dangerous creativity. Exact DTO names may be refined in `plan.md`, but endpoint purpose and security behavior must not be silently changed.
...
Authenticated response shape should remain compatible with current frontend auth state.

Unauthenticated response must be safe and must not throw.
...
* Success redirect: `/app/auth/verified?status=success`
* Expired redirect: `/app/auth/verified?status=expired`
* Invalid redirect: `/app/auth/verified?status=invalid`
...
Response should be safe and user-friendly.
...
Must clear session, security context, and remember-me.
...
* Starts backend-managed Google OAuth2 flow.
* Frontend button links to this endpoint.
...
* USER redirect: `/app/home`
* ADMIN redirect: `/app/admin`

Failure behavior:

* Redirect to `/app/auth?oauthError=...` or a dedicated safe auth error state.

---

## Auth Error Codes

The feature uses a limited set of stable auth error codes:
...
* Codes are for frontend behavior and tests.
* Messages must be localized.
* Internal exception names, stack traces, SQL errors, provider secrets, and token hashes must never be exposed.
...
13. Registration, forgot password, and resend verification require captcha.
14. Rate limits reject abusive resend and forgot-password attempts.
15. Resend sends email in production configuration and logs links in dev when configured.
16. Admin User Details → Account tab shows email verification status.
17. Auth and landing texts show no raw i18n keys and are natural in EN/RU.
18. No PDF/AI/generation/finalization files are modified.
19. Legacy custom auth/CSRF/interceptor code is removed after Spring Security proof.
20. Backend and frontend tests pass with added security coverage.
...
* Add only necessary Spring Security integration.
* Keep JDBC DAO style.
* Avoid giant all-in-one services.
* Avoid speculative auth abstractions.

### Testing Excellence

* TDD required for meaningful security behavior.
* Tests must cover success, failure, boundary, and abuse cases.
* Migration must include regression tests for existing login/status/logout behavior.
* Existing unrelated tests must not be weakened.

### User Experience Consistency

* Keep SPA auth experience.
* Keep `/api/auth/status` frontend compatibility.
* Add clear verification/reset/OAuth/captcha states.
* Preserve role redirects.
* Keep copy bilingual and natural.

### Performance & Reliability

* Use simple database-backed tokens.
* Use rate limits for public auth endpoints.
* Do not introduce heavy frameworks.
* Avoid network calls in tests by mocking Resend, Turnstile, and Google OAuth behavior.

### Security by Design

* Backend Spring Security is authoritative.
* No raw tokens in database.
* No secrets in frontend assets.
* No sensitive fields in DTOs.
* No permanent double-auth system.
* No production silent no-op for email/captcha.
* No captcha-as-DDoS claim for landing.

---

## Open Questions

No known product-level open questions after brainstorm session 2026-06-29.

If implementation discovers a conflict with the current codebase, STOP and report:

1. exact file/class/method involved;
2. requirement that conflicts;
3. safest options;
4. recommendation;
5. no code changes until user confirms.
