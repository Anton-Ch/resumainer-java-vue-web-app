...
**Note**: Existing route stub in `GenerateResumeController` already uses `/candidate/{publicCode}` pattern. The spec originally referenced `/{username}/{publicCode}` from BA docs, but the codebase already settled on `/candidate/` prefix. Implementation reuses this existing stub — replace 501 placeholder with real PDF serving.
...
- Apply IP-based rate limiting on `GET /candidate/{publicCode}`.
