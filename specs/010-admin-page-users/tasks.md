...
> **Instruction for implementer**: This task file is intentionally strict. Follow task order, stop at every `[STOP]`, use MCP tools as required, and do not expand scope. If current code conflicts with this task list, STOP and report the conflict before editing. Do not silently reinterpret requirements.
...
- Do not add a modal-details API endpoint unless current architecture absolutely requires it and the user explicitly approves (per FR-135).
...
- [ ] T009 [SERENA] Inspect DAO models/tables used for users, contact details (with `full_name` and `resume_email`), additional info, resumes, work experience, education, projects, courses.
...
- [ ] T016 [STOP] [EVIDENCE] Report baseline findings:
  ...
  - contact_detail has `full_name` and `resume_email` columns confirmed;
...
- [ ] T020 [SEC] Implement backend ADMIN authorization for every `/api/admin/**` endpoint. Use `request.getServletPath()` for context-path-safe path matching (per plan safety note).
- [ ] T021 [SEC] Ensure frontend route guard is not treated as backend security.
- [ ] T022 [KISS] Create `AdminController`, `AdminService`, and `AdminDao` following existing layered patterns. Keep admin code clearly separated.
...
- [ ] [SERENA] Inspect existing `PagedResponse` (field `items`), `HomeSavedResumeDto`, resume mappers, and User Home DAO queries.
...
- [ ] T026 [TDD] Add tests for `GET /api/admin/dashboard` returning real total users (non-deleted) and total resumes (non-deleted), plus WIP token stats as `0`.
...
- [ ] T030 [TDD] Add tests for admin resumes search across resume title, vacancy title, company name, owner username, owner email (`users.email`), owner full name (`contact_detail.full_name` via LEFT JOIN).
...
- [ ] T035 Implement `AdminSavedResumeDto` with owner fields (`ownerEmail` from `users.email`, `ownerFullName` from `contact_detail.full_name` via LEFT JOIN) and canonical safe resume action URLs.
...
- [ ] T037 Implement admin resumes DAO query with pagination, search, filters, date range, whitelist sorting, and LEFT JOIN contact_detail for ownerFullName.
...
- [ ] T040 [STOP] [EVIDENCE] Report:
  ...
  - proof owner fields exist with `ownerFullName` from contact_detail;
...
- [ ] T048 [NO-PDF-AI] Confirm no changes to public route/PDF generation are made. Verify that existing `410 Gone` behavior for soft-deleted resume public URLs still applies after admin delete.
...
- [ ] T049 [STOP] [EVIDENCE] Report:
  ...
  - confirmation public URLs for deleted resume still return 410 (existing behavior);
...
- [ ] T051 [TDD] Add tests for shared text search across `users.username`, `users.email`, `contact_detail.full_name`.
...
- [ ] T059 Implement `AdminUserListItemDto` with `fullName` from `contact_detail.full_name`, `email` from `users.email`.
- [ ] T060 Implement DAO/service/controller for `/api/admin/users` with LEFT JOIN contact_detail for fullName.
...
- [ ] T064 [STOP] [EVIDENCE] Report:
  ...
  - proof `fullName` comes from contact_detail;
  - test command output.
...
**Goal**: Implement `/api/admin/users/{userId}` read model for Contacts, Account, Additional sections.
...
- [ ] T069 Implement DTOs:
  - `AdminUserDetailsDto`;
  - `AdminUserContactsDto` (with `resumeEmail` from `contact_detail.resume_email`, `fullName` from `contact_detail.full_name`);
  - `AdminUserAccountDto` (with `accountEmail` from `users.email` as read-only field);
  - `AdminUserAdditionalDto`.
- [ ] T070 Implement DAO/service projection for Contacts fields:
  - full name (`contact_detail.full_name`);
  - professional title;
  - `resumeEmail` (`contact_detail.resume_email`);
  - phone;
  - location;
  - LinkedIn;
...
- [ ] T071 Implement DAO/service projection for Account fields:
  - username;
  - `accountEmail` (`users.email`, read-only);
  - role;
  - status;
...
- [ ] T074 [STOP] [EVIDENCE] Report:
  - user details API sample showing `resumeEmail` in contacts, `accountEmail` in account;
...
- [ ] T081 [TDD] [SEC] Add tests proving `PATCH /api/admin/users/{userId}/access` on soft-deleted or non-existent user returns `404 Not Found` (FR-092a).
- [ ] T082 Implement `AdminAccessUpdateRequestDto`.
- [ ] T083 Implement access update service/DAO using lookup codes, not IDs.
- [ ] T084 [SEC] Ensure non-admin cannot update access.
- [ ] T085 [KISS] Use clear validation errors internally, generic localized frontend message externally.

### User Soft-Delete Tasks

- [ ] T086 [TDD] Add tests for successful `DELETE /api/admin/users/{userId}` on another user.
- [ ] T087 [TDD] [SEC] Add tests rejecting self-delete.
- [ ] T088 [TDD] Add tests that user delete sets `users.is_deleted = true` and `users.deleted_at`.
- [ ] T089 [TDD] Add tests that user status becomes `BLOCKED` after delete.
- [ ] T090 [TDD] Add tests that user's saved resumes are soft-deleted.
- [ ] T091 [TDD] Add tests for repeatable profile record soft-delete only where schema supports it.
- [ ] T092 [TDD] Add tests that contact/additional profile records are not migrated/forced into new soft-delete behavior.
- [ ] T093 [TDD] [SEC] Add tests for non-existent/already-deleted user returning safe generic response.
- [ ] T094 [TDD] [SEC] Add test verifying that after user soft-delete cascade, previously valid public URLs for the deleted user's resumes follow existing `410 Gone` behavior and do not serve active PDFs (FR-105a).
- [ ] T095 Implement user soft-delete transaction:
  - mark user deleted;
  - set deleted_at;
  - set status to BLOCKED;
  - soft-delete saved resumes;
  - soft-delete repeatable profile records with existing soft-delete fields.
- [ ] T096 [SEC] Do not hard-delete any rows.
- [ ] T097 [SEC] Do not create audit log.
- [ ] T098 [SEC] Do not add migrations unless user explicitly approved earlier.

### Checkpoint

- [ ] T099 [STOP] [EVIDENCE] Report:
  - access update tests;
  - self-protection tests (self-delete, self-demotion, self-block rejection);
  - deleted user PATCH returns 404 test;
  - user delete transaction behavior;
  - public link 410 after cascade verification;
...
## Phase 7 — Frontend Routing, Services, Types, and i18n Foundation
...
- [ ] T100 [TDD] Add/update router tests if project has router tests; otherwise document why not and cover through component/e2e tests later.
- [ ] T101 Add routes:
  - `/admin` existing Admin Home;
  - `/admin/users`;
  - `/admin/users/:userId`;
  - `/admin/ai-models` WIP.
- [ ] T102 Ensure routes require admin via existing frontend route guard (`meta: { requiresAuth: true, requiresAdmin: true }`).
- [ ] T103 Create frontend admin types for dashboard, saved resume, user list item, user details (with `resumeEmail` and `accountEmail`), access update request.
- [ ] T104 Create `adminService.ts` methods:
  - getDashboard;
  - listAdminResumes;
  - deleteAdminResume;
  - listAdminUsers;
...
- [ ] T105 Add i18n keys in `en.json` and `ru.json` for all new admin pages, filters, labels, statuses, permissions, rights, WIP badges, toasts, confirmations, tooltips, empty states, `Resume email` / `Email for resume` label.
- [ ] T106 [SEC] Ensure no frontend type expects sensitive fields.
- [ ] T107 [KISS] Keep composables page-specific unless reuse is clearly simple.

### Checkpoint

- [ ] T108 [STOP] [EVIDENCE] Report:
  - routes added;
  - service methods;
  - i18n keys added;
...
- [ ] T109 [TDD] Add tests for Admin Home dashboard rendering real stats and WIP badges.
- [ ] T110 [TDD] Add tests for quick actions:
  - Users navigates to `/admin/users`;
  - Resumes scrolls/anchors to Admin Resumes section;
  - AI Models navigates to `/admin/ai-models`.
- [ ] T111 Build/update `AdminHomePage.vue` dashboard area.
- [ ] T112 Build `AdminStatsCards.vue` if it keeps page simpler.
- [ ] T113 Build `AdminQuickActions.vue` if it keeps page simpler.
- [ ] T114 [TDD] Add tests for Admin Resumes table filters/search/date range/pagination reset.
- [ ] T115 [TDD] Add tests blocking `createdTo` earlier than `createdFrom`.
- [ ] T116 [TDD] Add tests for reset filters behavior.
- [ ] T117 [TDD] Add tests for row click opening modal and hover/clickable classes.
- [ ] T118 Build `AdminResumesTable.vue` using User Home table style and behavior.
- [ ] T119 Wire Admin Resumes table into `/app/admin` directly.
- [ ] T120 Ensure Admin Resumes table does not live on separate `/app/admin/resumes` page.
- [ ] T121 Wire row click to existing/resuable `ResumeDetailsDialog`.
- [ ] T122 Add admin delete resume flow in modal context:
  - confirmation;
  - loading/disabled button;
  - success reload;
  - failure keeps modal open.
- [ ] T123 Ensure filters/sort/pagination state is local to Admin Home table.
- [ ] T124 [SEC] Ensure modal does not render raw paths or sensitive fields.

### Checkpoint

- [ ] T125 [STOP] [EVIDENCE] Report:
...
- [ ] T126 [TDD] Add tests for Admin Users page rendering table columns.
- [ ] T127 [TDD] Add tests for one shared search field over username/email/fullName behavior at service query level.
- [ ] T128 [TDD] Add tests for account creation date filter label and date range behavior.
- [ ] T129 [TDD] Add tests preventing `createdTo` earlier than `createdFrom`.
- [ ] T130 [TDD] Add tests for role/status/permission/rights filters.
- [ ] T131 [TDD] Add tests for page reset on filter/search changes.
- [ ] T132 [TDD] Add tests for reset filters button.
- [ ] T133 [TDD] Add tests for sort interaction and whitelist query values sent to service.
- [ ] T134 [TDD] Add tests for row click navigation to `/admin/users/:userId`.
- [ ] T135 Build `AdminUsersPage.vue`.
- [ ] T136 Build `AdminUsersTable.vue` if it keeps page simpler.
- [ ] T137 Implement columns exactly:
  - full name;
  - username;
  - email;
  - role;
  ...
  - resumes count;
  - created date.
- [ ] T138 Ensure no action/open-details column exists.
- [ ] T139 Match User Home row hover/highlight and pointer behavior.
- [ ] T140 Ensure controls/pagination clicks do not trigger row navigation.
- [ ] T141 Add localized empty state.

### Checkpoint

- [ ] T142 [STOP] [EVIDENCE] Report:
...
- [ ] T143 [TDD] Add tests for Admin User Details loading and tab order: Contacts, Account, Additional, Resumes.
- [ ] T144 [TDD] Add tests for not-found/deleted user state.
- [ ] T145 Build `AdminUserDetailsPage.vue`.
- [ ] T146 Build `AdminUserDetailsShell.vue` or reuse existing profile shell style if simpler.
- [ ] T147 [TDD] Add tests for Contacts tab fields and read-only behavior (including `resumeEmail` label).
- [ ] T148 Build `AdminUserContactsTab.vue` showing `resumeEmail` from `contact_detail.resume_email`, UI label MUST be `Resume email` / `Email for resume`.
- [ ] T149 [TDD] Add tests for Additional tab fields and read-only behavior.
- [ ] T150 Build `AdminUserAdditionalTab.vue`.
- [ ] T151 Ensure missing fields show localized empty state, not null/undefined.
- [ ] T152 Ensure long text wraps safely.

### Account Tab

- [ ] T153 [TDD] Add tests for Account tab controls:
  - role;
  - status;
  - generation permission;
  - rights/isPrivileged;
  - read-only `accountEmail` displayed.
- [ ] T154 [TDD] Add tests Save disabled until changes exist.
- [ ] T155 [TDD] Add tests Save confirmation/cancel/success/failure flows.
- [ ] T156 [TDD] Add tests unsaved changes warning on tab switch/navigation/close/back (only for Account tab dirty state).
- [ ] T157 [TDD] Add tests self-delete disabled tooltip for current admin.
- [ ] T158 [TDD] Add tests frontend prevents/warns on self-demotion and self-block where practical.
- [ ] T159 [TDD] Add tests own generation permission and own privileged flag remain editable.
- [ ] T160 Build `AdminUserAccountTab.vue` with editable controls + read-only `accountEmail`.
- [ ] T161 Wire update access service call with loading state and toasts.
- [ ] T162 Wire delete user confirmation/loading/toast/redirect.
- [ ] T163 Ensure delete account button is danger secondary.
- [ ] T164 Ensure own delete button is disabled with tooltip.
- [ ] T165 Ensure backend remains source of truth for self-protection.

### Resumes Tab

- [ ] T166 [TDD] Add tests for selected user resumes table showing only selected user's resumes.
- [ ] T167 [TDD] Add tests no Create Resume button exists.
- [ ] T168 [TDD] Add tests filters/sort/pagination state is independent from Admin Home table.
- [ ] T169 [TDD] Add tests row click opens resume modal.
- [ ] T170 [TDD] Add tests admin delete resume reloads only selected user's resumes table unless broader reload is active.
- [ ] T171 Build `AdminUserResumesTab.vue` using same table behavior as admin/home resumes table where simple.
- [ ] T172 Ensure Resumes tab has search/filter/sort/pagination and date range validation.
- [ ] T173 Ensure no Create Resume button.
- [ ] T174 Wire modal and admin resume delete flow.

### Checkpoint

- [ ] T175 [STOP] [EVIDENCE] Report:
...
- [ ] T176 [TDD] Add test for `/admin/ai-models` rendering localized WIP page.
- [ ] T177 Build `AdminAiModelsWipPage.vue` or reuse existing placeholder component.
- [ ] T178 Ensure no AI model CRUD controls exist.
- [ ] T179 Ensure no API key/provider configuration details are displayed.
- [ ] T180 Ensure route requires admin.

### Checkpoint

- [ ] T181 [STOP] [EVIDENCE] Report:
...
- [ ] T182 Run backend targeted tests for admin controller/service/DAO.
- [ ] T183 Run full backend test suite. Use the existing project command, for example `mvn test` or the repository-specific Maven wrapper command. If the full suite cannot run, STOP and report the blocker.
- [ ] T184 Confirm new/changed backend code coverage target 80%+ or report measured scope coverage and legacy baseline.

### Frontend Verification

- [ ] T185 Run frontend targeted Vitest tests for new/changed admin code.
- [ ] T186 Run full available frontend test suite using the existing project command (e.g. `npm run test:unit` or the repository-specific equivalent). If the full suite cannot run, STOP and report the blocker.
- [ ] T187 Run frontend build command if the project has one (e.g. `npm run build` or equivalent). If the build command fails, STOP and report the blocker.
- [ ] T188 Run frontend coverage if available.
- [ ] T189 Confirm new/changed frontend code coverage target 80%+ or report measured scope coverage and legacy baseline.

### Playwright MCP Flows

- [ ] T190 [PW-MCP] Login as admin.
- [ ] T191 [PW-MCP] Open `/app/admin` and capture screenshot.
- [ ] T192 [PW-MCP] Verify dashboard stats and WIP token badges.
- [ ] T193 [PW-MCP] Verify Admin Resumes table search/filter/date/sort/reset/pagination.
- [ ] T194 [PW-MCP] Verify Admin Resumes row click opens modal.
- [ ] T195 [PW-MCP] Verify admin resume delete confirmation/cancel path.
- [ ] T196 [PW-MCP] Verify admin resume delete success path on disposable test data if available.
- [ ] T197 [PW-MCP] Open `/app/admin/users` and capture screenshot.
- [ ] T198 [PW-MCP] Verify users search/filter/date/sort/reset/pagination.
- [ ] T199 [PW-MCP] Verify user row click opens details page.
- [ ] T200 [PW-MCP] Capture Contacts tab screenshot (verify `resumeEmail` label).
- [ ] T201 [PW-MCP] Capture Account tab screenshot (verify read-only `accountEmail`).
- [ ] T202 [PW-MCP] Verify unsaved changes warning on Account tab.
- [ ] T203 [PW-MCP] Verify self-delete disabled tooltip for current admin account.
- [ ] T204 [PW-MCP] Verify access update confirmation/cancel path.
- [ ] T205 [PW-MCP] Verify access update success path on disposable non-admin user if available.
- [ ] T206 [PW-MCP] Verify delete user confirmation/cancel path.
- [ ] T207 [PW-MCP] Verify delete user success path on disposable test user if available.
- [ ] T208 [PW-MCP] Capture Additional tab screenshot.
- [ ] T209 [PW-MCP] Capture Resumes tab screenshot (verify no Create Resume button).
- [ ] T210 [PW-MCP] Verify Resumes tab row click modal and delete resume path on disposable data if available.
- [ ] T211 [PW-MCP] Open `/app/admin/ai-models` and capture WIP screenshot.

### Postgres MCP Verification

- [ ] T212 [PG-MCP] If delete tests used real/disposable data, verify soft-delete flags in DB.
- [ ] T213 [PG-MCP] Verify self-delete/self-demotion/self-block are not persisted after rejected attempts if applicable.
- [ ] T214 [PG-MCP] Verify no hard deletes occurred in tested flows.

### Checkpoint

- [ ] T215 [STOP] [EVIDENCE] Report:
  - backend test output (full suite command and output);
  - frontend test output (full suite command and output);
  - frontend build output if available (command and output);
...
- [ ] T216 [SERENA] Inspect all changed files and references for unused code, dead branches, accidental duplication, and stale imports.
- [ ] T217 [SEC] Audit admin DTOs for sensitive fields (`password_hash`, API keys, raw paths).
- [ ] T218 [SEC] Audit SQL sorting for whitelist only.
- [ ] T219 [SEC] Audit self-protection rules in backend tests and implementation.
- [ ] T220 [SEC] Audit delete flows: soft-delete only, no hard delete.
- [ ] T221 [SEC] Verify `PATCH /api/admin/users/{userId}/access` on deleted user returns `404 Not Found`.
- [ ] T222 [SEC] Verify `DELETE /api/admin/resumes/{resumeId}` and user soft-delete cascade preserve existing `410 Gone` behavior for public URLs.
- [ ] T223 [NO-PDF-AI] Confirm no PDF/AI/generation pipeline files were modified.
- [ ] T224 Confirm no new dependencies were added unless previously approved.
- [ ] T225 Confirm no migrations were added unless previously approved.
- [ ] T226 Confirm no `/app/admin/resumes` full page exists.
- [ ] T227 Confirm no AI Models CRUD exists.
- [ ] T228 Confirm all new visible strings are in EN/RU i18n files.
- [ ] T229 Confirm no raw i18n keys appear in browser screenshots.
- [ ] T230 Confirm code follows KISS and removes overengineered abstractions introduced during implementation.
- [ ] T231 [MEMORY] If valuable lessons emerged, propose memory update text and wait for user confirmation.

### Final Checkpoint

- [ ] T232 [STOP] [EVIDENCE] Final implementation report:
  - completed scope;
  - changed files grouped by backend/frontend/tests/docs;
  - full backend test command and output;
  - full frontend test command and output;
  - frontend build command and output, if available;
  - targeted coverage or scoped coverage explanation for new/changed code;
...
- Contacts tab shows `resumeEmail` from `contact_detail.resume_email` with correct label.
- Account tab shows read-only `accountEmail` from `users.email`.
- Admin can soft-delete resumes.
- Admin can soft-delete another user.
- Admin cannot self-delete, self-demote, or self-block.
- `PATCH /api/admin/users/{userId}/access` on deleted user returns `404 Not Found`.
- Soft-deleted resumes/users preserve existing `410 Gone` behavior for public URLs.
